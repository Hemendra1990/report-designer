...
export function DataTable<TData extends Record<string, any>>(props: DataTableProps<TData>) {
    ...
    const isPivotColumn = (columnId: string) => {
      ...
      return pivotColumns.some((pc: string) => columnId.includes(pc));
    };
              ...
              <thead className="bg-slate-50 text-slate-600 sticky top-0 z-10 shadow-sm">
                {isPivotTable && (
                  <tr className="pivot-header-row">
                    <th 
                      colSpan={table.getLeftHeaderGroups()[0]?.headers.length || 1}
                      className="px-4 py-2 text-left text-xs font-semibold uppercase tracking-wider border-b border-slate-200"
                    >
                      Group By Fields
                    </th>
                    <th 
                      colSpan={pivotColumns.length > 0 ? pivotColumns.length : 1}
                      className="px-4 py-2 text-center text-xs font-semibold uppercase tracking-wider border-b border-slate-200 bg-blue-50"
                    >
                      Pivot Data ({pivotValues.length} values)
                    </th>
                  </tr>
                )}
                      ...
                      return (
                        <th 
                          ...
                          className={`px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider whitespace-nowrap border-b border-slate-200 first:rounded-tl-md ${
                            isPivot ? 'bg-blue-50/80 text-blue-700' : ''
                          }`}
                        ...
                        const cellStyle = cell.getIsGrouped()
                          ...
                          : cell.getIsAggregated()
                            ...
                            : cell.getIsPlaceholder()
                              ? ''
                              : isPivot 
                                ? 'bg-blue-50/40 font-medium text-blue-800'
                                : '';
